<#
.FUNCTIONALITY
This is a VMWare tools install script that re-attempts the install if it finds that the VMWARE tools service has failed to install on the first attempt
.SYNOPSIS
- This script can be used as part of automating Windows 10/Server2019 builds via autounattend.xml with packer.io based builds
- The Packer instance requires the VMWare tools service to be running at the end of the build, else, it will fail
- Due to an issue Windows "VMware tools service" failing to install on the first attempt, the code in this script completes a re-install of the VMWARE tools package
#>
...
write-host "Starting VMware tools install attempt 1" -ForegroundColor cyan
...
while (-not$Running -and $iRepeat -lt 5) {
    ...
    write-host "Pause for 2 seconds to check running state of VMware tools service" -ForegroundColor cyan 
    Start-Sleep -s 2
